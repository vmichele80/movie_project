...
def add_movie(movies):
    ...
    if movie_title not in movies:
        ...
        print(f"""The movie {movie_title} has been added.
            Rating {movies[movie_title]['rating']},
            Year of release {movies[movie_title]['year']}""")
    else:
...
def update_movie(movies):
    ...
    if movie_to_update in movies:
        movies[movie_to_update]["rating"] = float(input("Enter new movie rating: "))
        print(f"The rating for the movie {movie_to_update} has been updated.")
    else:
        print("Movie not found")
...
def main():
    ...
    while True:
        ...
        if user_choice == "1":
            ...
            update_movie(movies_list)
            movies_storage.save_movies(movies_list)
        elif user_choice == "5":
            stats(movies_list)
        elif user_choice == "6":
            ...
            movies_sorted_by_rating(movies_list)
        elif user_choice == "0":
            movies_storage.save_movies(movies_list)
            exit_app()
        else:
